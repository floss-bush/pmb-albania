...
// si par un quelconque hasard, on se retrouve apr�s le dernier enregistrement, rechargement de la liste au premier ouvrage
...
//Renvoi un tableau contenant la liste des groupes, ainsi que l'index du premier �l�ment de ce groupe dans la requete.
...
if( $nbpages_arrondi != 0 && empty($numero_page)) {
 	...
 	$navpage .= '&numero_page=1">'.$msg[49].' ></a>'; // on passe la variable numero page � 1
		...
		$navpage .= '<a href="'.$page.'?categ='.$categ.'&sub='.$sub.'&limite_page='.$limite_page.'&numero_page='.$precedent;
 		$navpage .= '">< '.$msg[48].'</a>'; // retour page pr�c�dente
