...
// +-------------------------------------------------+
// $Id: keyword.inc.php,v 1.35 2010-11-17 17:15:24 arenou Exp $
...
if($clause) {
	...
	if($opac_visionneuse_allow){
		...
		$req_bull = "select distinct typdoc, count(explnum_id) as nbexplnum from notices left join bulletins on bulletins.num_notice = notice_id and bulletins.num_notice != 0 left join explnum on explnum_bulletin = bulletin_id and explnum_bulletin != 0 and explnum_mimetype in ($opac_photo_filtre_mimetype) $statut_j $acces_j $clause group by typdoc";
		$req_typdoc="select distinct typdoc, sum(nbexplnum) as nbexplnum from ($req_bull union $req_noti) as uni group by uni.typdoc"; 
	}
