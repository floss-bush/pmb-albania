			...
			while($row = mysql_fetch_object($check)){
				...
				if(count($value['distrib']) == 0){ //Si le titre que je veux ajouter n'a pas de distribution je regarde si celui qui exsite en a une
					...
					if (mysql_num_rows($test)) {
						$different = true; //Si il � une distribution le titre que je veux ajouter est diff�rent
					}
					...
					if($nb != count($value['distrib'])){ //Si il y en a pas autant c'est un titre diff�rent
						...
						for($i=0;$i<count($value['distrib']);$i++) {
							...
							if (!$nb) {
								$different = true; //Si une des distributions n'existe pas c'est un titre uniforme diff�rent
							}elseif($nb != $nb_occurence[$value['distrib'][$i]]){
								$different = true; //Si le nombre de cette distribution est diff�rent c'est un titre uniforme diff�rent
							}
						}
					}
				}
				...
				if(count($value['ref']) == 0){ //Si le titre que je veux ajouter n'a pas de r�ference je regarde si celui qui exsite en a une
					...
					$test = mysql_query($requete, $dbh);
					if (mysql_num_rows($test)) {
						$different = true; //Si il � une r�ference le titre que je veux ajouter est diff�rent
					}
					...
					if($nb != count($value['ref'])){ //Si il y en a pas autant c'est un titre diff�rent
						...
						for($i=0;$i<count($value['ref']);$i++) {
							...
							$nb=@mysql_result($test,0,0);
							if (!$nb) {
								$different = true; //Si une des r�ference n'existe pas c'est un titre uniforme diff�rent
							}elseif($nb != $nb_occurence[$value['ref'][$i]]){
								$different = true; //Si le nombre de cette r�ference est diff�rent c'est un titre uniforme diff�rent
							}
						}
					}
				}
				if($different == false){ //Si le titre n'est pas diff�rent on retourne l'id du titre identique
					return $tu_id;
				}
			}
			return $tu->tu_id;
