...
// +-------------------------------------------------+
// $Id: category_browse.php,v 1.37 2010-07-28 07:44:39 mbertin Exp $
//
...
$base_url = "./category_browse.php?caller=$caller&p1=$p1&p2=$p2&no_display=$no_display&bt_ajouter=$bt_ajouter&dyn=$dyn&keep_tilde=$keep_tilde&parent=";
...
if(!$nbr_lignes) {
	if(!$user_input) {
		...
		if ($id_thes && $id_thes != -1) $requete.= "where num_thesaurus = '".$id_thes."' ";
		else if($id_thes == -1) $requete.="where num_thesaurus in (select id_thesaurus from thesaurus)";
...
if($nbr_lignes) {
	if ($user_input) {
		...
		if($id_thes == -1){
			...
			$nb_cat_results = mysql_result($result_nb, 0, 0);
			...
			if($nb_cat_results>$nbmax_cat){
				$requete.= "limit ".$debut.",".$nbmax_cat." ";
				$mv_pagine = true;
			}
		}
	}
...
if((mysql_num_rows($result))&&($nbr_lignes)) {
	...
	if (!$user_input) {
		...
		$browser_content .= "<a href='#' $java_comment onclick=\"set_parent('$caller', '$tcateg->id', '".htmlentities(addslashes($nom_tesaurus.$tcateg->catalog_form),ENT_QUOTES, $charset)."')\">";
		if($id_thes == -1){
			$browser_content .= '['.htmlentities($tcateg->thes->libelle_thesaurus,ENT_QUOTES, $charset).']';
		} else {
			$browser_content .= '';
		}
		...
		if(sizeof($tcateg->path_table) && $id_thes !=-1) {
			...
			$id_aj = $tcateg->path_table[sizeof($tcateg->path_table) - 1]['id'] ;
		} else {
			$browser_header = "";
			$t = thesaurus::getByEltId($cat[0]);
			$id_aj = $t->num_noeud_racine;
		}
	}
	while($cat = mysql_fetch_row($result)) {
		...
		if ((($keep_tilde!=1)&&(($tcateg->catalog_form[0]!="~")||($tcateg->voir_id)))||($keep_tilde)) {
			...
			if($tcateg->voir_id) {
				$temp = new category($tcateg->voir_id);
				$display .= "$tcateg->libelle -&gt;<i>".$temp->catalog_form."@</i>";
			} else {
			...
			if ($thesaurus_mode_pmb && $caller=='notice') $nom_tesaurus='['.$tcateg->thes->getLibelle().'] ' ;
 			...
 			$browser_content .= "<a href='#' $java_comment onclick=\"set_parent('$caller', '$id_', '".htmlentities(addslashes($nom_tesaurus.$libelle_),ENT_QUOTES, $charset)."')\">";
	...
	if($mv_pagine){
		...
		$nav_bar = aff_pagination ($url_base, $nbr_lignes, $nbmax_cat, $page, 10, false, true) ;
		print $nav_bar;
	} else if($id_thes == -1 || $user_input){
		//Création barre de navigation
		$url_base=$base_url.'&categ=categories&sub=search&id_thes='.$id_thes.'&user_input='.rawurlencode(stripslashes($user_input));
